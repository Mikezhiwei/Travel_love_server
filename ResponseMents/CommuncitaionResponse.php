...
include_once("../JSONUtils/isInert.php");
 class  CommentReponse {
 	       ...
 	       public function response_comments($com_user_id,$com_reply_id,$msg_id)//评论人的ID或者是回复人的ID的顺序其实都差不多
           {
        	    ...
        	    $this->comment_mapper->doGetCluster(array('com_user_id'=>$com_user_id,
                                                           'com_reply_id'=>$com_reply_id));       	
                $pos=0;
                $arr=array();
           ...
           public function response_get_comments_one_all($msg_id)
           {
           	      $this->comment_mapper->do_onemessage_allcomments($msg_id);
           	      $pos=0;
           	      $arr=array();
           	      foreach($this->comment_mapper->GetCollection() as $value)
           	      {
