...
class  Communication_Mapper extends Mapper{
             ...
             private static $SELECT_SQL2="select t1.nickname,t2.nickname,comments.com_detail,comments.com_time,comments.com_id
                                          ...
                                          order by comments.com_time;"; //查询一个消息的所有评论者和回复者的昵称
             ...
             public function do_onemessage_allcomments($msg_id)
             {
                $this->select_statme2->bind_param('i',$msg_id);
                $this->select_statme2->execute();
                $this->select_statme2->store_result();
                $this->select_statme2->bind_result($com_name,$reply_name,$comment_detail,$comments_time,$com_id);
                while($this->select_statme2->fetch())
                {
                	    $tempraw=array( 'com_id'=>$com_id,
                	                    'commentname'=>$com_name,
                	                    'replyname'=>$reply_name,
                	                    'com_detail'=>$comment_detail,
                	                    'com_time'=>$comments_time,
                	                    'msg_id'=>$msg_id
                	                    );
                	    $this->collection->add($this->CreateObject($tempraw));
                }
